...
public class SignListener implements Listener {
    ...
    public void onSignChange(SignChangeEvent event) {
        System.out.println("Created sign is " + ((Integer.valueOf(event.getBlock().getData()) * 0.0625) * 360) + " degrees off of 0");

        // make sure the sign creation is suppose to be for us at all
        if (!event.getLine(0).equalsIgnoreCase("[pipe]")) return;

        // make sure player has permission to be building pipettes
        ...
        // get requested pipette mode from line 3
        PipetteMode mode = PipetteMode.parse(event.getLine(2));
        ...
        // create pipette
        Pipette createdPipette = new Pipette(targets, mode, ownerName, ownerUuid);

        DevathonPlugin.instance.getLogger().info("Player " + event.getPlayer().getName() + " has created a pipette going towards " + directions + " at " + new PrettyLocation(event.getBlock().getLocation()));
    }
